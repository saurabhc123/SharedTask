...
def _get_constituents(parse_dict, connective):
    ...
    #print "Connective Indices: " + str(conn_indices);
    constituent_nodes = []
...
def test_maxent(algorithms, train, test, flag):
     ...
     for algorithm, classifier in classifiers.items():
         ...
         #print "Accuracy: " + str(nltk.classify.util.accuracy(classifier, test));
	 count = 0;
...
def prepRel(inFile, outFile):
 ...
 print "Number of SS Explicit: " + str(countSS); 
 #with open(outFile, 'w') as f:
 #    json.dump(orel, f)
...
def readInput(inputFilenamePath, trainOrTest):
 ...
 if trainOrTest == "test":
  print "Reading ss_relations.json"; 
...
def preprocessing(inputFilenamePath):
 ...
 print "Dictionary: " + str(filenameSentenceSentToken);

def splitSSandPS(inputFilenamePath, trainOrTest, observedArray):
 print "In function: splitSSandPS";
 parse_file = codecs.open(inputFilenamePath+'/parses.json', encoding='utf8');
 en_parse_dict = json.load(parse_file);
...
def SS_parallel_not_parallel(inFile, trainOrTest, oa):
 print "In function: SS_parallel_not_parallel";
 ...
 for dictEntry in ss_array:
  ...
  print "conn_indices: " + str(conn_indices);
  parallel = False;
 ...
 print "Size of Parallel List: " + str(len(SS_conns_parallel_list));
 print "Size of Non parallel List: " + str(len(SS_conns_not_parallel_list));
 ...
 print "Number of Connectives: " + str(len(connectives));
 ...
 for curr_index, connective in enumerate(connectives):
  ...
  print "sentenceIndexForConstituents: " + str(sentenceIndexForConstituents);
  print "Size of SS_conns_not_parallel: " + str(len(SS_conns_not_parallel_list));
  ...
  for i, constituent in enumerate(constituents):
   ...
   print "Arg1 Orig List: " + str(connective.Arg1_token_indices);
   print "Arg2 Orig List: " + str(connective.Arg2_token_indices);
   ...
   print "Constituent Indices: " + str(constIndices);

   if constIndices.issubset(arg1OrigSet):
    label = "arg1";
    count1= count1 + 1;
    print "Constituent is part of arg1";
   elif constIndices.issubset(arg2OrigSet):
    label = "arg2";
    count2 = count2 + 1;
    print "Constituent is part of arg2";
   else:
    label = "null";
    count3 = count3 + 1;
    print "Constituent is null";
...
def ssOrPS(inputFilenamePath):
 ...
 en_parse_dict = json.load(parse_file);
 i = 0;
 print "Size of Observed Array: " + str(observedArray);
 print "big diction: " + str(bigDiction);
 print "Token Diction: " + str(dictDocumentToken);
 with open('/home/development/code/explicit_args/output_dev.json', 'w') as f:
  ...
  for prediction in observedArray:
   ...
   print "sentenceNumber: " + str(sentenceNumber); 
   print "Sentence Index: " + str(wordList);
   ...
   if prediction == 'PS':
    ...
    if len(arg1TokenList) <> 0:
     print "Length of Arg1: " + str(len(arg1TokenList));
    ...
    if len(arg2TokenList) <> 0:
     print "Length of Arg2: " + str(len(arg2TokenList));
...
def assignSSandPS(inputFilenamePath): 
 ...
 for x in observedArray:
  print "Dictionary: " + str(bigDiction[i]);
  print "Length of Tuple: " + str(len(bigDiction[i]));
  print "i: " + str(i);
  prediction = x;
...
def mergeSS(constituents, predictedArray, conns_list, parse_dict):
 print "Connective Array Length: " + str(len(constituents));
 print "Predicted Array Length: " + str(len(predictedArray));
 ...
 print "Size of Relation Dictionary: " + str(len(relation_dict));
 ...
 source = "SS"
 index = 0;
 print "Size of Connective List: " + str(len(conns_list));
 print "Size of Relations: " + str(len(relation_dict));
 for i, dictEntry in enumerate(conns_list):
  ...
  if relation_dict.has_key(i):
   ...
   print "Arg1_list Length: " + str(len(Arg1_list));
   print "Arg2_list Length: " + str(len(Arg2_list));
   if Arg1_list != [] and Arg2_list != []:
    print "Connective: " + str(conn_indices);
    print "Arg1: " + str(Arg1_list);
    print "Arg2: " + str(Arg2_list);
...
def printToFile(outFile, temp, parse_dict):
 print "Size of temp file: " + str(len(temp));
 #print "SentToDocDict: " + str(sentToDocToken);
 index = 0;
 '''
 with open('/home/development/code/explicit_args/rel.json', 'w') as f:
  for data in temp:
   dictEntry = dict();
   relID = index;
   docID = data[1];
   ...
   f.write("\n");
 ''' 
 with open('/home/development/code/explicit_args/connless.txt', 'w') as t:
  for data in temp:
   docID = data[1];
   conn_indices = get_doc_offset(parse_dict, docID, data[2],data[3]);
...
if __name__ == '__main__':
 ...
 trConn, trSet, SS_conns_parallel_list, SS_conns_not_parallel_list, parse_dict = SS_parallel_not_parallel('/home/development/data/conll16st-en-01-12-16-train', 'train', []);
 ...
 tConn, tSet, SS_conns_parallel_list, SS_conns_not_parallel_list, parse_dict = SS_parallel_not_parallel('/home/development/data/conll16st-en-01-12-16-dev', 'test', oa);
 ...
 print "Size of Feature Set: " + str(len(tSet));
 ...
 printToFile('/home/development/code/explicit_args/dev_out_newf.json', temp, parse_dict); 
