...
def getAllVerbsFromData(dictByDocID):
    ...
    for docID in dictByDocID:
        for sentID in dictByDocID[docID]:
            ...
            (verbs, tags) = getVerbsFromClause(
                dictByDocID, docID, sentID, wordIDs)
            for v in verbs:
...
def _ps_arg1_clauses(parse_dict, relation, Arg):
    ...
    sent_tokens = [(index, parse_dict[DocID]["sentences"][sent_index][
                    "words"][index][0]) for index in range(0, sent_length)]
    ...
    parse_tree = parse_dict[DocID]["sentences"][
    ...
    conn_token_indices = [item[4]
                          for item in relation["Connective"]["TokenList"]]
...
def getTokenIDsList(relation, key):
    result = []
    # each token that is part of Arg1 is represented as a list of 5 indices; I
    # just need to take 3rd index (tokenID)
    for tokenList in relation[key]['TokenList']:
...
def get_curr_production_rule(parse_dict, docID, sentID, conn_indices, clause):
    ...
    syntax_tree = Syntax_tree(parse_tree)
    if syntax_tree.tree != None:
        clause_leaves = set([syntax_tree.get_leaf_node_by_token_index(
            index) for index in curr_clause_indices])
        no_need = []
    ...
    for tree in subtrees:
        for node in tree.traverse(strategy="levelorder"):
            if not node.is_leaf():
                rule = node.name + "-->" + \
                    " ".join([child.name for child in node.get_children()])
...
def getVerbFeatures(dictByDocID, docID, sentID, clauseWordIDs, verbList):
    (allVerbs, tags) = getVerbsFromClause(
        dictByDocID, docID, sentID, clauseWordIDs)
    verbs = []
    for v in allVerbs:
        if v in verbList:
            verbs.append(v)
...
def modifyProductionRules3(productionRules):
    newRules = []
    posTags = ['NN', 'NNP', 'ADJ', 'VBP', 'VBD', 'JJ', 'JJR', 'PRP', 'NNS',
               'NNPS', 'VBZ', 'VB', 'RB', 'VBN', 'VBG', 'VBZ', 'CD', 'MD', 'JJS', 'PRP$']
...
def filterProductionRules(productionRules):
    newRules = []
    posTags = ['NN', 'NNP', 'ADJ', 'VBP', 'VBD', 'JJ', 'JJR', 'PRP', 'NNS', 'NNPS',
               'VBZ', 'VB', 'RB', 'VBN', 'VBG', 'VBZ', 'CD', 'JJS', 'PRP$', '-RRB-', 'DT', '-LRB-']
    for r in productionRules:
        pair = r.split('->')
        lhs = pair[0].strip()
